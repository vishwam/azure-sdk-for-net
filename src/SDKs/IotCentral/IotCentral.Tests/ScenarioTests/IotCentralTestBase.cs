...
namespace IotCentral.Tests.ScenarioTests
{
    ...
    public class IotCentralTestBase
    {
        ...
        protected IotCentralClient iotHubClient;
            ...
            if (!initialized)
            {
                lock (locker)
                {
                    if (!initialized)
                    {
                        ...
                        iotHubClient = IotCentralTestUtilities.GetIotCentralClient(context, new RecordedDelegatingHandler { StatusCodeToReturn = HttpStatusCode.OK });
        ...
        protected App CreateIotCentral(ResourceGroup resourceGroup, string location, string appName)
        {
            ...
            return this.iotHubClient.Apps.CreateOrUpdate(
                resourceGroup.Name,
                appName,
                app);
        }
